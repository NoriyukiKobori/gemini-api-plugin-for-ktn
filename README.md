...
- パッケージングしたプラグインは作者のブログで公開しています。
  - https://hazime-style.com/?p=3585
...
- kintoneの複数のフィールドの値からプロンプトを生成
- 生成したプロンプトはリクエスト前にダイアログ上で編集が可能
- Gemini APIへリクエストを送信
...
- 複数フィールドからプロンプトを生成できるように改修
- 生成したプロンプトをリクエスト前に編集できるように改修。
- リクエスト前の確認画面をSweetAlert2からkintone UI Componentベースへ変更。
- その他、エラーハンドリング調整。
...
本プラグインは hazime-plugin-template-for-ktn v1.2.0 をベースに開発されています。
